...
myfile = st.file_uploader("Choose a file", accept_multiple_files=False,
                            type=["fits"], key="1")
...
datfile = st.file_uploader("Choose a dat", accept_multiple_files=False, type=["dat"], key="2")
if datfile is not None:
    data = np.loadtxt(datfile, dtype=[('timestamp', 'M8[us]'), ('setpoint', float), ('fforward', float), ('metrology', float), ('error', float), ('stage', float), ('piezo', float)])
else:
    st.write("No file uploaded")
    exit()
...
onetime = (data["timestamp"]-data["timestamp"][0]).astype(float)
master_time = np.arange(onetime[crop_samples], np.max(onetime), 250.)[:-crop_samples+1]
...
MAN_DL_commands = fforward[crop_samples:][:-crop_samples, None]#Resampling DL positions
dl_exts = []
dl_cols = []
...
DL_positions = metrology[crop_samples:][:-crop_samples, None]
print("DL_positions:", DL_positions.shape)
print("MAN_DL_commands:", MAN_DL_commands.shape)
print("master_time", master_time_s.shape)
print("onetime", onetime.shape)
print(onetime[:4])
print(master_time[:4])
